package org.cfm.java.jdbc.repositorio;
...
public class ProductoRepositorioImpl implements Repositorio<Producto> {
    ...
    public Producto porId(Long id) {
        ...
        try(PreparedStatement stmt = getConnection().prepareStatement("SELECT p.*, c.nombre AS categoria FROM productos AS p INNER JOIN " +
                "categorias AS c ON (p.categoria_id = c.id) WHERE p.id = ?")) {
            stmt.setLong(1, id);
